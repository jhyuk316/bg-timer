...
- 설정 화면의 `싱글 | 멀티` 모드 전환
- 방 생성과 영문 대문자·숫자 4자리 방 코드 발급
- QR 코드 또는 방 코드로 참가
- 한 기기 참가자가 여러 플레이어 말을 선택
- 모든 기기를 합쳐 1~6개의 플레이어 말 사용
- 준비 완료와 방장 시작
- 전체 보드와 타이머 실시간 동기화
- 방장과 게스트 모두 모든 말 조작
- 방장 전용 게임 시작과 종료
...
방 ID는 추측하기 어려운 Firebase push ID를 사용한다. 방 코드는 영문 대문자 `A-Z`와 숫자 `0-9`로 구성된 4자리 문자열이다. `roomCodes` 생성은 transaction으로 충돌을 확인한 뒤 확정하며 이미 사용 중인 코드이면 새 코드를 만들어 다시 시도한다.
...
2. 수동 참가자는 4자리 코드를 입력한다. 입력창은 소문자를 즉시 대문자로 표시한다.
3. 조회 전 입력값에 `trim().toUpperCase()`를 적용하고 `[A-Z0-9]{4}` 형식인지 검사한다.
4. 존재하며 로비 상태인 방만 참가할 수 있다.
5. 참가 성공 시 현재 방 ID와 정규화된 대문자 코드를 localStorage에 저장한다.

### Lobby

- 모든 기기는 플레이어 말, 소유 기기, 참가자 연결·준비 상태를 실시간 구독한다.
- 각 기기는 여러 말을 선택할 수 있다.
- 총 선택 수가 1~6개일 때만 게임을 시작할 수 있다.
...
- 1~6명 모두 모바일 가로 화면에서 줄바꿈 없는 한 줄 레이아웃을 사용한다.
- 6명은 동일 폭 6칸으로 표시하며 이름, 타이머와 상태 문구가 각 칸 안에 들어오도록 제한된 폭에 맞춘 크기를 사용한다.
...
- 방 코드 생성 문자와 길이
- 소문자 방 코드 입력의 대문자 정규화와 잘못된 형식 거부
- 시작 가능 조건: 1~6개 말, 모든 연결 참가자 준비
- 모든 방 참가자의 말 조작 권한
- 이벤트 revision과 상태 전이
...
- 1명부터 6명까지 한 줄 레이아웃 유지와 6명에서 이름·타이머 가독성 확인
- 세로 방향 회전 안내 확인
...
- 여러 기기가 총 1~6개의 말을 충돌 없이 나누어 맡는다.
- 방장 3개, 게스트 1개 말의 4인 게임을 시작하고 끝낼 수 있다.
- 6개의 말을 선택한 게임이 모바일 가로 화면 한 줄 레이아웃으로 시작하고 끝난다.
