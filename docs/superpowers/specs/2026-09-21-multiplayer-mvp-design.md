...
- `multiplayer-game.js`: 원자적인 턴 이벤트, 일시정지, 재개, 종료 및 시간 파생
...
rooms/{roomId}
  ...
  game
    ...
    events/{eventId}
      revision
      type                start | switch | referee | pause | resume | end
      actorUid
      playerId
      at
```
...
- 게임 시작, 일시정지, 재개와 종료는 방장만 수행한다.
- 실행 중 턴 변경은 방장이거나 새 활성 말의 `ownerUid`와 일치하는 게스트만 요청할 수 있다.
...
- 방장은 연결이 끊긴 게스트의 말도 계속 조작할 수 있다.
...
- 자신의 말은 강조하고 다른 기기의 말은 약하게 표시하되 이름과 시간은 동일하게 읽을 수 있게 한다.
- 게스트에게 담당하지 않은 말은 비활성화한다.
- 방장은 모든 말을 활성 상태로 표시하고 조작할 수 있다.
- 연결이 끊긴 기기의 말에는 작은 연결 끊김 표시를 추가한다.
- 일시정지와 종료 버튼은 방장에게만 표시한다.
...
- 턴 변경, 운영 시간, 일시정지·재개와 종료 동기화
- 게스트 네트워크 차단 후 방장의 계속 진행
...
- 게스트는 담당 말만, 방장은 모든 말을 조작한다.
- 게스트 연결이 끊겨도 방장이 게임을 진행하고 재접속이 복구된다.
- 모든 기기의 현재 턴, 시간, 일시정지와 종료 상태가 수렴한다.
