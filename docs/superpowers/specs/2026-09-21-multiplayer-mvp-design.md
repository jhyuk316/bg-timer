...
2. 수동 참가자는 숫자 6자리 코드를 입력한다. 입력창은 `inputmode="numeric"`을 사용해 모바일 숫자 키패드를 연다.
3. 화면에는 `381 204`처럼 3자리씩 나누어 표시할 수 있지만 조회 전 공백을 제거하고 `[0-9]{6}` 형식인지 검사한다.
4. 존재하며 로비 상태인 방만 참가할 수 있다.
5. 참가 성공 시 현재 방 ID와 앞자리 `0`을 포함한 6자리 문자열 코드를 localStorage에 저장한다.
...
- 방 코드 생성 문자와 길이
- 표시용 공백을 제거한 방 코드 정규화, 앞자리 `0` 보존과 잘못된 형식 거부
