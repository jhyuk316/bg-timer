...
- **총 시간**: 플레이어별 총 시간 (턴 간 유지, 내부 변수: reserveTime)
- **턴 딜레이**: 매 턴 새로 부여되는 기본 시간 (내부 변수: turnTime)
- **패널티 추가시간**: 총 시간 소진 시 패널티와 함께 부여되는 추가 시간
...
1. 턴 시작 → 턴 딜레이 카운트다운
2. 턴 딜레이 소진 → 총 시간 카운트다운 진입 (시각적 전환)
3. 총 시간 소진 → **패널티 -2%** 적용 + 추가 총 시간 부여
4. 반복마다 패널티 누적 (-2%, -4%, -6%, ...)
...
- 턴 딜레이 → 총 시간 전환 시 경고 (색상 변화 + 선택적 소리)
- 총 시간 잔여 5분 시점에 TTS 음성 알림 (Web Speech API, 예: "5분 남았습니다")
- 총 시간 잔여 30% 시점에 추가 경고
- 총 시간 소진 시 패널티 알림 (화면 플래시 + 소리)
- 패널티 누적 횟수 상시 표시
- 소리 on/off 토글
