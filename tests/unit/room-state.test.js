...
test('requires every connected participant to be ready', () => {
  ...
  assert.equal(canStartGame(room, 6_000), true);
});

test('removes a lobby guest from presence after five seconds', () => {
  const room = makeRoom({ playerCount: 2, guestReady: false });
  room.participants.host.lastSeenAt = 20_000;
  room.participants.guest.lastSeenAt = 1_000;

  assert.equal(isParticipantPresent(room.participants.guest, 5_999), true);
  assert.equal(isParticipantPresent(room.participants.guest, 6_000), false);
  assert.deepEqual(getStaleLobbyParticipantUids(room, 6_000), ['guest']);
  assert.equal(canStartGame(room, 6_000), false);
